...
API for the base image(s) of an OCI-compliant tarball.

To produce an OCI-compliant tarball, run `docker image save <image id>` with a
docker client version v25.0 or above.
*/
package main
...
func main() {
	log.SetFlags(0)
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: base_image_query <image.tar>\n")
		flag.PrintDefaults()
	}
